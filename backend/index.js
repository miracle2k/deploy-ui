...
const {extClient, coreClient} = require('./k8s');
...
app.post('/api/deployment/*', async (req, res) => {
  ...
  const deployment = await extClient.ns(namespace).deployments(resource).getPromise();
  const newImage = patchDockerUrl(deployment.spec.template.spec.containers[0].image, {tag: newTag});

  extClient.ns(namespace).deployments(resource).patch({
    body: {
      spec: {
        template: {
          spec: {
...
app.get('/api/deployment/*', async (req, res) => {
  ...
  const deployment = await extClient.ns(nsName).deployments(deploymentName).getPromise();
  ...
  if (deployment.spec.template.spec.imagePullSecrets) {
    ...
    // Get the secret
    const secret = await coreClient.ns(nsName).secret(secrets[0].name).getPromise();
    dockercfg = JSON.parse(Buffer.from(secret.data['.dockercfg'], 'base64').toString("ascii"));
  }
